...
class ThrowingTask:
    ...
    def _handle_throwing_phase(self):
        ...
        2. Once velocity > 1.0 m/s, release immediately and stop robots
        ...
        if self._throw_debug_counter % 50 == 0:
            print(f"Throwing: vel={vel_magnitude:.3f} m/s, vel_vec={current_vel}, contact={contact}")
            print(f"  Target vel: {self.throw_release_velocity}")

        # Check if box has achieved sufficient throwing velocity
        if vel_magnitude > 1.0:  # Box is moving fast enough (successful throw)
            print(f"🚀 Box released! Velocity: {vel_magnitude:.3f} m/s - Robots stopping immediately.")

            # DISABLE collisions so box can separate from robots
        ...
        # CORRECT APPROACH: DS computes desired END-EFFECTOR velocity (not object velocity!)
        # The DS output tells us how fast we want the robot hands to move in the throw direction

        # Use DS controller with box velocity to ramp up smoothly
        ee_vel_desired = self.ds_controller.compute_desired_velocity(
            self.throw_release_pos,  # Use fixed release position
            current_vel,  # Current box velocity (for smooth ramping)
            self.throw_release_pos,  # Same as "current" position = zero position error
            self.throw_release_velocity,  # Target throw velocity
            self.model.opt.timestep
        )

        # Increase forward velocity to ensure box moves forward more aggressively
        forward_velocity = np.array([3.0, 0.0, 0.0])  # Increased to 3.0 m/s forward
        self.throw_release_velocity += forward_velocity

        # Adjust DS controller's desired velocity to include forward motion
        ee_vel_desired += forward_velocity * 0.5  # Scale forward velocity for smoother ramp-up

        # Debug: Print adjusted desired velocity and joint velocities
        print(f"Adjusted DS ee_vel: {ee_vel_desired}, box_vel: {current_vel}")

        # Map END-EFFECTOR velocity to joint velocities using Jacobian
        try:
            # Get Jacobians for both end-effectors
            jac_left = np.zeros((6, self.model.nv))
            jac_right = np.zeros((6, self.model.nv))
            mujoco.mj_jacSite(self.model, self.data, jac_left[:3], jac_left[3:], self.left_ee_id)
            mujoco.mj_jacSite(self.model, self.data, jac_right[:3], jac_right[3:], self.right_ee_id)

            # Get arm DOFs
            left_dofs, right_dofs = self.jacobian_mapper.get_arm_dofs()

            # Extract relevant Jacobian columns (only linear part)
            jac_left_arm = jac_left[:3, left_dofs]
            jac_right_arm = jac_right[:3, right_dofs]

            # Compute joint velocities for both arms to achieve desired EE velocity
            qdot_left = np.linalg.pinv(jac_left_arm) @ ee_vel_desired
            qdot_right = np.linalg.pinv(jac_right_arm) @ ee_vel_desired

            # Debug: Print joint velocities
            print(f"Joint velocities - Left: {qdot_left}, Right: {qdot_right}")

            # Combine
            joint_velocities = np.zeros(self.num_actuators)
            joint_velocities[:self.actuators_per_robot] = qdot_left
            joint_velocities[self.actuators_per_robot:] = qdot_right
            ...
            print(f"Error in throwing Jacobian: {e}")
            return np.zeros(self.num_actuators)
