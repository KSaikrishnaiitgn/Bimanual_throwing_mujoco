"""
This is the main file for the modular throwing task using MuJoCo. It sets up the simulation environment,
initializes various controllers, and orchestrates the throwing process through different phases: reaching,
reading sensors, lifting, and throwing. The code leverages a velocity controller to command joint velocities
based on the current phase of the task.
"""
import os
...
class ThrowingTask:
    ...
    def _handle_throwing_phase(self):
        """Handle throwing phase logic - matches throwing_fixed.py exactly"""
        ...
        # Check contact - if lost, stop commanding
        _, _, contact = self.contact_handler.get_object_contact_wrenches(self.box_body_id)

        if not contact:
            # Object released naturally → stop commanding arms
            return np.zeros(self.num_actuators)
