        ...
        if control_state["phase"] == "reaching_first_target" or control_state["phase"] == "reaching_second_target":
            ...
            if current_box_height >= control_state["target_lift_height"] - height_threshold:
                ...
                if not control_state.get("throw_phase_started", False):
                    ...
                    # Small time margin before actual release (seconds of sim time)
                    control_state["throw_release_time"] = data.time + 0.05
            ...
            """
            Phase: throwing_object

            Goal:
            - Perform a throwing motion by moving the robot hands in the throw direction
            - This imparts momentum to the box through contact forces
            - Release the box at the right moment to achieve the desired trajectory
            """
            box_body_id = control_state["box_body_id"]
            dof_adr = model.body_dofadr[box_body_id]  # start index in qvel for this body's free joint
            sim_time = data.time

            # Initialize throwing motion if not done
            if "throw_motion_started" not in control_state:
                control_state["throw_motion_started"] = False
                control_state["throw_motion_complete"] = False
                control_state["throw_retract_complete"] = False
                control_state["throw_motion_start_time"] = sim_time

                # Get throw direction from release velocity
                v_release = control_state["throw_release_velocity"]
                throw_dir = v_release / (np.linalg.norm(v_release) + 1e-6)
                control_state["throw_direction"] = throw_dir

                print(f"\n🎯 Starting throw motion in direction: [{throw_dir[0]:.3f}, {throw_dir[1]:.3f}, {throw_dir[2]:.3f}]\n")

            # Step 0: Perform throwing motion - push box forward with robot arms
            if not control_state["throw_retract_complete"]:
                throw_push_duration = 0.15  # duration of forward throwing push
                time_elapsed = sim_time - control_state["throw_motion_start_time"]

                if time_elapsed < throw_push_duration:
                    # THROW: Push hands forward in throw direction while holding box
                    try:
                        ...
                        if left_ee_site_id == -1 or right_ee_site_id == -1:
                            control_state["throw_retract_complete"] = True
                            return np.zeros(num_actuators)
                        ...
                        # THROW: Move both arms forcefully in throw direction
                        throw_dir = control_state["throw_direction"]
                        throw_speed = 5.0  # Very fast throwing motion (m/s)

                        # Desired end-effector velocity in throw direction
                        desired_ee_vel_3d = throw_dir * throw_speed
                        desired_ee_vel_6d = np.zeros(6)
                        desired_ee_vel_6d[:3] = desired_ee_vel_3d

                        # Compute joint velocities using Jacobian pseudoinverse
                        qdot_left = np.linalg.pinv(jac_left_arm) @ desired_ee_vel_6d
                        qdot_right = np.linalg.pinv(jac_right_arm) @ desired_ee_vel_6d

                        # Combine into full velocity command
                        ...
                        # Limit to safe velocities
                        max_vel = 5.0
                        throw_velocity = np.clip(throw_velocity, -max_vel, max_vel)

                        return throw_velocity

                    except Exception as e:
                        print(f"Error in retraction: {e}")
                        control_state["throw_retract_complete"] = True
                        return np.zeros(num_actuators)
                else:
                    # Throwing push complete - now RELEASE the box!
                    control_state["throw_retract_complete"] = True

                    # Store initial position for tracking
                    control_state["throw_start_pos"] = data.xpos[box_body_id].copy()

                    # Get current box velocity from the throwing motion
                    box_dof_adr = model.body_dofadr[box_body_id]
                    current_box_vel = data.qvel[box_dof_adr : box_dof_adr + 3].copy()

                    # Disable ALL collisions to release the box cleanly
                    print("🔓 Releasing box - disabling all robot collisions...")
                    for i in range(model.ngeom):
                        geom_name = model.geom(i).name
                        if geom_name and "box" not in geom_name.lower() and "ground" not in geom_name.lower() and "floor" not in geom_name.lower():
                            model.geom_contype[i] = 0
                            model.geom_conaffinity[i] = 0

                    print(f"\n🚀 THROW COMPLETE! Box released with velocity: [{current_box_vel[0]:.3f}, {current_box_vel[1]:.3f}, {current_box_vel[2]:.3f}] m/s")
                    print(f"   Starting position: [{control_state['throw_start_pos'][0]:.3f}, {control_state['throw_start_pos'][1]:.3f}, {control_state['throw_start_pos'][2]:.3f}]")
                    print(f"   Target: [{LANDING_POINT[0]:.3f}, {LANDING_POINT[1]:.3f}, {LANDING_POINT[2]:.3f}]\n")

                    control_state["throw_motion_complete"] = True
                    return np.zeros(num_actuators)

            # Step 1: Perform throwing motion - push hands in throw direction
            if not control_state["throw_motion_complete"]:
                throw_duration = 0.1  # short throwing motion

                if not control_state["throw_motion_started"]:
                    control_state["throw_motion_started"] = True
                    print(f"\n🎯 Starting throwing motion with contacts enabled...\n")

                time_elapsed = sim_time - control_state["throw_motion_start_time"]

                if time_elapsed < throw_duration:
                    # Use Jacobian to compute joint velocities for desired end-effector motion
                    try:
                        # Find end-effector site IDs
                        left_ee_site_id = -1
                        right_ee_site_id = -1
                        ...
                        # Desired end-effector velocity (in throwing direction)
                        throw_dir = control_state["throw_direction"]
                        ee_vel_magnitude = 3.0  # m/s - increased for more aggressive push
                        desired_ee_vel = throw_dir * ee_vel_magnitude

                        # Only use linear velocity (first 3 components)
                        desired_vel_6d = np.zeros(6)
                        desired_vel_6d[:3] = desired_ee_vel
                        ...
                        # Combine into full joint velocity vector
                        throw_velocity = np.zeros(num_actuators)
                        throw_velocity[:actuators_per_robot] = qdot_left
                        throw_velocity[actuators_per_robot:] = qdot_right
                        ...
                        print(f"Error computing throw velocity: {e}")
                        return np.zeros(num_actuators)
                else:
                    # Throwing motion complete - now release!
                    control_state["throw_motion_complete"] = True

                    # Disable contacts to release the box
                    for gname in [
                        "left_contact_sphere",
                        "left_palm_geom",
                        "right_contact_sphere",
                        "right_palm_geom",
                    ]:
                    ...
                    # Get current box velocity and boost it if needed
                    current_box_vel = data.cvel[box_body_id, 3:6].copy()  # linear velocity
                    print(f"\n🚀 RELEASE! Box velocity at release: [{current_box_vel[0]:.3f}, {current_box_vel[1]:.3f}, {current_box_vel[2]:.3f}] m/s\n")

                    return np.zeros(num_actuators)

            # Step 2: After release, keep arms passive and let box fly
            # Check if box has landed (velocity near zero and on ground)
            if "throw_landed" not in control_state:
                control_state["throw_landed"] = False
                control_state["throw_debug_counter"] = 0

            if not control_state["throw_landed"]:
                box_pos = data.xpos[box_body_id]
                box_dof_adr = model.body_dofadr[box_body_id]
                box_vel = data.qvel[box_dof_adr : box_dof_adr + 3]
                speed = np.linalg.norm(box_vel)

                # Print trajectory every 50 steps to see where box is going
                control_state["throw_debug_counter"] += 1
                if control_state["throw_debug_counter"] % 50 == 0:
                    print(f"📦 Box flying: pos=[{box_pos[0]:.3f}, {box_pos[1]:.3f}, {box_pos[2]:.3f}], vel=[{box_vel[0]:.3f}, {box_vel[1]:.3f}, {box_vel[2]:.3f}]")

                # Check if box has stopped moving (landed)
                if speed < 0.01 and box_pos[2] < 0.15:  # very slow and near ground
                    control_state["throw_landed"] = True
                    distance = np.linalg.norm(box_pos[:2] - control_state["throw_start_pos"][:2])
                    print(f"\n🎯 LANDED! Box traveled {distance:.3f}m horizontally")
                    print(f"   Start: [{control_state['throw_start_pos'][0]:.3f}, {control_state['throw_start_pos'][1]:.3f}, {control_state['throw_start_pos'][2]:.3f}]")
                    print(f"   End:   [{box_pos[0]:.3f}, {box_pos[1]:.3f}, {box_pos[2]:.3f}]")
                    print(f"   Target was: [{LANDING_POINT[0]:.3f}, {LANDING_POINT[1]:.3f}, {LANDING_POINT[2]:.3f}]\n")
    ...
    with mujoco.viewer.launch_passive(model, data) as viewer:
        while viewer.is_running():
            ...
            if box_body_id != -1:
                ...
                if hasattr(data, "cvel"):
                    lin_vel = data.cvel[box_body_id, 3:6].copy()
                else:
                    lin_vel = np.zeros(3)
